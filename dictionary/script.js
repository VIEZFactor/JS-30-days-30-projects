...
async function fetchDictionaryData(searchTerm){
    try {
        ...
        console.log(error);
        alert(`An error ocurred while searching ${searchTerm}`);
    }
}
        ...
        <ul>
            ${
                wordData.meanings.map((meaning) => `
                    <li>
                        <p><strong>Part of Speech: ${meaning.partOfSpeech}</strong></p>
                        <p><strong>Definition: ${meaning.definitions[0].definition}</strong></p>
                    </li>
                `).join('\n')
            }
        </ul>
